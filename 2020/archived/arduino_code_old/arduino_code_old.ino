...
void setup() {
 ...
 Serial.println("{\"cmd\":\"MQTT-SUB\",\"topic\":\"sub/topic/mouth\"}");
...
void loop() {
  ...
  if (stringComplete) {
    ...
    Serial.println("{\"cmd\":\"MQTT-PUB\",\"topic\":\"topic/status\",\"message\":\"ACK "+topic+"\"}");
    if (topic == "led") { 
      ...
      tP = cmd;
      //displayLED(tP.toInt(), states[tP.toInt()][0], states[tP.toInt()][1], states[tP.toInt()][2]);
      displayLED();
      //Serial.println("I'm a LED");
      }
    if (topic == "mouth") { 
      mouth = !mouth;
    ...
    if (topic == "motor") {
      if (cmd == "f" | cmd == "forward" | cmd == "F") { forward(); }
      if (cmd == "r" | cmd == "reverse" | cmd == "R") { reverse(); }
      if (cmd == "s" | cmd == "stop" | cmd == "S") { stop(); }
    }
  }
...
//void displayLED(int pin, int R, int G, int B) {
void displayLED() {
...
int light() {
  ...
  Serial.println("{\"cmd\":\"MQTT-PUB\",\"topic\":\"topic/ldr\",\"message\":\""+String(map(photor, 0, 1023, 100, 0))+"\"}");
...
Serial.println("{\"cmd\":\"MQTT-PUB\",\"topic\":\"topic/volt\",\"message\":\""+String(val2)+"\"}");
return(val2);
...
void resetState() {
  ...
  displayLED();
 //for (int i=0; i<8; i++) {
 // displayLED(i, states[i][0], states[i][1], states[i][2]);
 //}
}
