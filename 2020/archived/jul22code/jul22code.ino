...
//because we still need to be in the json object to get our led vaules we will check the topic here

if (topic == "sub/topic/led") { 
   
      for (int i=0; i<8; i++) {
        states[i] = root["message"][i].as<String>();
        
      }
      displayLED();
      
      } else { cmd = root["message"].as<String>(); }
