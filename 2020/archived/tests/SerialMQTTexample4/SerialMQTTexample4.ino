...
if (stringComplete) {
    ...
    if (topic == "count") {
      Serial.println("{\"cmd\":\"MQTT-PUB\",\"topic\":\"topic/status\",\"message\":"+String(count)+"}");
      count++;
    }
  }
}
...
void blink() {
  if (cmd == "1") {
    ...
    Serial.println("{\"cmd\":\"MQTT-PUB\",\"topic\":\"topic/status\",\"message\":\"ON\"}");
  }
  if (cmd == "0") {
    digitalWrite(LED_BUILTIN, LOW);
    Serial.println("{\"cmd\":\"MQTT-PUB\",\"topic\":\"topic/status\",\"message\":\"OFF\"}");
  }
}
